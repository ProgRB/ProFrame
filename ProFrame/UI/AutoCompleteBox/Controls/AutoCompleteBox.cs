    ...
    public partial class AutoCompleteBox : Control, IUpdateVisualState
    {
        ...
        private static void OnMinimumPopulateDelayPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (newValue < 0)
            {
                ...
                throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Невозможно установить значение {0}", newValue), "value");
            }
        ...
        private static void OnMaxDropDownHeightPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (newValue < 0)
            {
                ...
                throw new ArgumentException(string.Format("Неверное значение элемента - высота выпадающего списка = {0}", e.NewValue), "value");
            }
        ...
        #region MaxSearchResultCount
        public static readonly DependencyProperty MaxSearchResultCountProperty = DependencyProperty.Register(
                "MaxSearchResultCount",
                typeof(int),
                typeof(AutoCompleteBox),
                new PropertyMetadata(200));
        /// <summary>
        /// Максимальное кол-во найденных элементов в списке
        /// </summary>
        public int MaxSearchResultCount
        {
            get
            {
                return (int)GetValue(AutoCompleteBox.MaxSearchResultCountProperty);
            }
            set
            {
                SetValue(AutoCompleteBox.MaxSearchResultCountProperty, value);
            }
        }
        #endregion
        ...
        /// <summary>
        /// Показывает открыт ли выпадающий список контрола
        /// </summary>
        ...
        private static void OnSearchTextPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (!source._allowWrite)
            {
                ...
                throw new InvalidOperationException("Невозможно установить значение");
            }
        }
        ...
        /// <summary>
        /// Каким методм ищется элемет в списке. По умолчанию "начинается с текста"
        /// specified by the
        /// <see cref="P:System.Windows.Controls.AutoCompleteBox.ItemsSource" />
        /// property for display in the drop-down.
        /// </summary>
        /// <exception cref="T:System.ArgumentException">The specified value is not valid
        /// <remarks>
        /// Используется для режима подбора элементов
        /// Если требуется свой фильтр элементов, то указываете Фанктор ItemsFilter
        /// </remarks>
        ...
        public static readonly DependencyProperty FilterModeProperty =
            DependencyProperty.Register("FilterMode", typeof(AutoCompleteFilterMode), typeof(AutoCompleteBox),
                new PropertyMetadata(AutoCompleteFilterMode.StartsWith));

        #endregion public AutoCompleteFilterMode FilterMode

        #region public Func ItemsFilter
        /// <summary>
        /// Метод для отбора данных по списку
        /// <see cref="P:System.Windows.Controls.AutoCompleteBox.ItemsSource" />
        /// property for display in the drop-down.
        /// </summary>
        /// <value>Свой метод для отбора данных по 
        /// <see cref="P:System.Windows.Controls.AutoCompleteBox.ItemsSource" />
        /// свойству. По умолчанию не установлено</value>
        public Func<string, object, bool> ItemsFilter
        {
            get { return GetValue(ItemsFilterProperty) as Func<string, object, bool>; }
        ...
        public static readonly DependencyProperty ItemsFilterProperty =
            DependencyProperty.Register(
                ...
                typeof(AutoCompleteBox),
                new PropertyMetadata(OnItemsFilterPropertyChanged));
        ...
        /// <param name="e">Event arguments.</param>
        private static void OnItemsFilterPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            AutoCompleteBox source = d as AutoCompleteBox;
            ...
            if (value == null)
                if (source.FilterMode==AutoCompleteFilterMode.Custom)
                    source.FilterMode = AutoCompleteFilterMode.StartsWith;
            else
                source.FilterMode = AutoCompleteFilterMode.Custom;
        }

        #endregion public AutoCompleteFilterPredicate ItemsFilter

        #region public object Placeholder
        /// <summary>
        /// Надпись которая показывается, когда текст не введен никакой. Используется как приглашение или пояснение
        /// </summary>
        ...
        /// <summary>
        /// Адаптер служит как раз для фильтрации и представления списка отфильтрованных элементов
        /// </summary>
        ...
        /// <summary>
        /// Обработчик таймера когда стоит задержка показа
        /// </summary>
        /// <param name="sender">The source object.</param>
        ...
        /// </summary>
        /// <param name="value">Объект для форматирования</param>
        ...
        /// <summary>
        /// Уведомляет 
        /// <see cref="T:System.Windows.Controls.AutoCompleteBox" /> что свойство
        /// <see cref="P:System.Windows.Controls.AutoCompleteBox.ItemsSource" />
        /// было изменено и надо обновить список объектов поиска
        /// </summary>
        /// <remarks>
        /// Можно использовать по окончании длительной операции поиска
        /// </remarks>
        public void PopulateComplete()
        {
            ...
            // Генерируем события окончания загрузки данных
        ...
        private void UpdateTextCompletion(bool userInitiated)
        {
            ...
            // Обновлеяем выбранный элемент в котроле
        ...
        /// <param name="view">представление для фильтрации данных</param>
        /// <param name="predicate">Предикат фильтрации данных</param>
        /// <returns>Возвращает объект первый попавшийся или пустое значение</returns>
        private object TryGetMatch(string searchText, ObservableCollection<object> view, Func<string, string, bool> predicate)
        {
        ...
        /// <summary>
        /// Пробегаем все элементы для обновления представления списка
        /// </summary>
        private void RefreshView()
        {
            ...
            Debug.WriteLine("Режим фильтра="+FilterMode);

            // Cache the current text value
            string text = Text ?? string.Empty;

            bool objectFiltering = FilterMode == AutoCompleteFilterMode.Custom;
            bool inResults;

            Func<string, string, bool> TextFilter = AutoCompleteSearch.GetFilter(FilterMode);

            //Текущий индекс в результатах поиска
            int current_view_index = 0;
            //кол-во элементов в _view, где _view это как раз отфильрованые элементы (найденные уже)
            int view_count = _view.Count;
            List<object> items = _items;
            foreach (object item in items)
            {
                ///Если свой предикат то проверяем элементы по нему, иначе проверяем по установленому предикату (начинается, включает, регистр букв)
                inResults = objectFiltering ? ItemsFilter(text, item) : TextFilter(text, FormatValue(item));

                if (inResults && current_view_index<view_count && _view[current_view_index] == item) // если найденные элемент совпадает с текущим в пред. фильтрации, то оставляем его
                    current_view_index++;
                else if (inResults) // если же элемента нет в списке, то добавляем его
                {
                    // Insert the item
                    if (current_view_index<view_count)
                    {
                        // Unfortunately replacing via index throws a fatal 
                        // exception: View[view_index] = item;
                        // Cost: O(n) vs O(1)
                        _view.RemoveAt(current_view_index);
                        _view.Insert(current_view_index, item);
                        //_view[current_view_index] = item;
                        current_view_index++;
                    }
                    else
                    {
                ...
                else if (current_view_index<view_count)
                {
                    // Удаляем элемент из списка
                    _view.RemoveAt(current_view_index);
                    view_count--;
                }
                if (view_count >= MaxSearchResultCount)
                    break;
            }

            Debug.WriteLine(_view.Count);
        ...
        /// <summary>
        /// Обрабатываем когда ItemsSource свойство изменяется, и обновляем
        ...
        private void OnItemsSourceChanged(IEnumerable oldValue, IEnumerable newValue)
        {
            ...
            // Храним копию данных в виде списка
            _items = newValue == null ? null : new List<object>(newValue.Cast<object>().ToList());

            // Очищаем представление и устанавлниваем список для адаптера
            ClearView();
            if (SelectionAdapter != null && SelectionAdapter.ItemsSource != _view)
            {
                SelectionAdapter.ItemsSource = _view;
